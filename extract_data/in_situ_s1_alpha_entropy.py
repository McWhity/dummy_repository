...
class S1Observations(object):
    ...
    def get_band_data(self, timestep, band):
        ...
        fname = 'NETCDF:"{:s}":{:s}'.format(this_file, "localIncidenceAngle")
        obs_ptr = reproject_image(fname, self.state_mask)
...
for pixels in pixel:
    ...
    path_data = '/media/nas_data/Thomas/S1/processed/MNI_2017_new_final/pol_decomp_2'
    ...
    for field in fields:
        ...
        for esu in esus:
                ...
                if tim < datetime.datetime(2017, 3, 20) or tim > datetime.datetime(2017, 10, 20):
                    ...
                    if tim >= df_LAI.index.min() and tim <= df_LAI.index.max():
                        try:
                            ...
                            data = sentinel1_observations.get_band_data(tim, 'Entropy')
                            data_vh = sentinel1_observations.get_band_data(tim, 'Alpha')
                            ...
                            entropy[tim] = sigma_vv
                            anisotropy[tim] = sigma_vh
            ...
            np.save(os.path.join(save_path, 'entropy'+pixels+field+'.npy'), entropy)
            np.save(os.path.join(save_path, 'alpha'+pixels+field+'.npy'), anisotropy)
